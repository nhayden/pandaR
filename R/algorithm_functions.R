...
#' @keywords keywords
#' @export
#' @return An object of class "panda" containing matrices describing networks achieved by convergence with PANDA algorithm.\cr
...
panda <- function( motif,
  ...
  while(hamming_cur>hamming){
    ...
    if(progress)
      cat("Iteration", step,": hamming distance =", round(hamming_cur,5),"\n")
    else
      cat('.')
    step=step+1
  }
 ...
 return(prepResult(zScale, output, regulatoryNetwork, geneCoreg, tfCoopNetwork))
}
...
prepResult <- function(zScale, output, regulatoryNetwork, geneCoreg, tfCoopNetwork){
  ...
  res <- pandaObj(regNet=regulatoryNetwork,
                   coregNet=geneCoreg,
